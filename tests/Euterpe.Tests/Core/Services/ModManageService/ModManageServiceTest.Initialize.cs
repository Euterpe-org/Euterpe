...
public sealed partial class ModManageServiceTest
{
    ...
    {
        const string modPath = "/mods/MyMod.dll";
        var localServiceMock = ILocalService.Mock();
        localServiceMock.GetModFilePaths().Returns([modPath]);
        localServiceMock.GetLibFilePaths().Returns([]);
        localServiceMock.LoadModFromPathAsync(modPath).Returns(CreateInstalledMod());
